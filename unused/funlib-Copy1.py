...
def eigN_density_fun(Omega, delta2, param):
    '''
    Returns the density function of the right-side integrand of the eigenvalue equation (0,infty)
    with respect to x (phase diff) and z (complex eigenvalue).
    '''
    ...
    gauss = lambda x: (2*pi*delta2)**(-1/2)*np.exp(-x**2/(2*delta2))
    tauE = lambda x: tau0 + gain*x
    f = lambda x, z: np.cos(-Omega*tauE(x) + x)*np.exp(-z*tauE(x))*gauss(x)
 
    return f


def eigN_pow_fun(Omega, delta2, param, deg):
    '''
    Returns the power term of the density function of the right-side integrand of the eigenvalue equation (0,infty)
    with respect to x (phase diff).
    '''
    ...
    gauss = lambda x: (2*pi*delta2)**(-1/2)*np.exp(-x**2/(2*delta2))
    tauE = lambda x: tau0 + gain*x
    f = lambda x: np.cos(-Omega*tauE(x) + x)*x**deg*gauss(x)
